-- ============================================================
-- BuzzardFrames: Options_PreviewSystem.lua
--
...
--
-- Depends on: Options_PreviewData.lua (ApplyPreview* functions)
-- ============================================================
...
-- Options_PreviewData.lua own that state).
...
-- Imports from Options_PreviewData.lua
...
-- Darken a preview frame with a black overlay at high frame level.
local function DesaturatePreviewBar(bar, desat)
    -- For preview, we darken the whole frame, not individual bars.
    -- This function is kept for API compatibility but is a no-op;
    -- the actual darkening is done at the frame level in applyOORAlpha.
end

function BF:DarkenPreviewFrame(frame, desat)
    if not frame then return end
    if desat > 0 then
        if not frame._oorDarkenFrame then
            local df = CreateFrame("Frame", nil, frame)
            df:SetAllPoints(frame)
            df:SetFrameLevel(frame:GetFrameLevel() + 50)
            df:EnableMouse(false)
            local tex = df:CreateTexture(nil, "OVERLAY", nil, 7)
            tex:SetAllPoints(df)
            tex:SetColorTexture(0, 0, 0, 1)
            df._tex = tex
            frame._oorDarkenFrame = df
        end
        frame._oorDarkenFrame._tex:SetAlpha(desat)
        frame._oorDarkenFrame:Show()
    else
        if frame._oorDarkenFrame then
            frame._oorDarkenFrame:Hide()
        end
    end
end
...
local function GetOptionsFrame()
    ...
    return aceFrame and aceFrame.frame and aceFrame.frame:IsShown() and aceFrame.frame or nil
end
...
local function MakePreviewFrame(name, parent, w, h)
    ...
    if not f.aggroHighlight then
        local function MakeEdge(par)
            local t = par:CreateTexture(nil, "OVERLAY")
        ...
        f.aggroHighlight.blizzardBorder = f.aggroHighlight:CreateTexture(nil, "OVERLAY")
        f.aggroHighlight.blizzardBorder:SetAllPoints(f.aggroHighlight)
        f.aggroHighlight.blizzardBorder:SetAlpha(0)
        ...
        f.aggroHighlight.cornersTex = cornersFrame:CreateTexture(nil, "OVERLAY")
        ...
        local arrowTex = (f.textFrame or f.aggroHighlight):CreateTexture(nil, "OVERLAY")
    ...
    if not f.dispelDebuffOverlay and f.healthBar then
        local dispelDebuffOverlay = f.healthBar:CreateTexture(nil, "OVERLAY", nil, 1)
    ...
    if not f.resIcon then
        ...
        statusIconFrame:SetFrameLevel(f:GetFrameLevel() + 21)
        ...
        for _, key in ipairs({"resIcon", "phasedIcon", "summonIcon", "rezPendingIcon", "vehicleIcon"}) do
            local ico = statusIconFrame:CreateTexture(nil, "OVERLAY")
        ...
        rcFrame:Hide()
        local rcTex = rcFrame:CreateTexture(nil, "ARTWORK")
        rcTex:SetAllPoints(rcFrame)
        f.readyCheckIcon = rcFrame
        f.readyCheckIconTexture = rcTex
    end

    -- Private aura dummy slots.
    -- Frame level +20 matches the real-frame PA wrapper in PrivateAuras.lua;
    -- this keeps preview rendering consistent with live frames where PAs
    -- sit above the health bar (+1) and aura icons (+16).
    if not f.dummyPrivateAuras then
        f.dummyPrivateAuras = {}
        for j = 1, 3 do
            local pa = CreateFrame("Frame", nil, f)
            pa:SetSize(14, 14)
            pa:SetFrameLevel(f:GetFrameLevel() + 31)
            local paTex = pa:CreateTexture(nil, "ARTWORK")
            paTex:SetAllPoints(pa)
            paTex:SetTexCoord(0.07, 0.93, 0.07, 0.93)
            pa.icon = paTex
            -- Rounded aura border (same as Blizzard buff icons), tinted red.
            -- The UI-TempEnchant-Border texture has transparent padding around
            -- its visible ring, so we must inflate the border outward beyond
            -- the pa frame for the ring to land at the icon edge.
            local border = pa:CreateTexture(nil, "OVERLAY")
            border:SetPoint("TOPLEFT", pa, "TOPLEFT", -3, 3)
            border:SetPoint("BOTTOMRIGHT", pa, "BOTTOMRIGHT", 3, -3)
            border:SetTexture(130870)  -- Interface\Buttons\UI-TempEnchant-Border
            border:SetVertexColor(1.0, 0.2, 0.2, 1)
            pa.border = border
            local paCd = CreateFrame("Cooldown", nil, pa, "CooldownFrameTemplate")
            paCd:SetPoint("TOPLEFT", paTex, "TOPLEFT", 0, -1)
            paCd:SetPoint("BOTTOMRIGHT", paTex, "BOTTOMRIGHT", 0, -1)
            paCd:SetDrawEdge(false)
            paCd:SetReverse(true)
            paCd:SetHideCountdownNumbers(false)
            paCd.timerText = paCd:GetCountdownFontString()
            if paCd.timerText then paCd.timerText:SetDrawLayer("OVERLAY", 7) end
            pa.cooldown = paCd
            pa:Hide()
            f.dummyPrivateAuras[j] = pa
        end
    end

    -- Pre-resolved per-frame anchor + power-bar visibility for the
    -- unified aura render path (RenderAuraGroup reads both).
    f._bf_auraAnchorTarget = f.healthBar and (f.healthBar.clipFrame or f.healthBar) or f
...
local function EnsurePreviewFramesContainer()
    ...
    cfc:SetFrameStrata("DIALOG")
    cfc:SetFrameLevel(120)
    cfc:Hide()
...
--
-- Must match the order produced by computeActivePreviewOrder in
-- Options_Preview.lua so the config tab and the preview frames
-- stay in lockstep.
-- ============================================================
...
-- Export so Options_PreviewData.lua can resolve a flat's position in
...
local function LayoutPreviewFrame(frame, flatID)
    ...
    local borderN = (bp.enableBorder ~= false) and (bp.borderThickness or 1) or 0
    ...
    if indicators then
        for i = 1, #indicators do
            local ind = indicators[i]
            if ind.GetFrame and ind:GetFrame(frame) then
                ind:Layout(frame)
            end
        end
    end
...
local function PositionPreviewCFFrames(p)
    ...
    cfc:SetHeight(panelFrame:GetHeight())
...
local function PositionPreviewFrames(activeOrder)
    ...
    c:SetHeight(panelFrame:GetHeight())
...
-- all preview frames every 30 seconds. Only runs when the Auras
-- section is open and showPreviewAuras is enabled.
-- ============================================================
...
function BF:ShouldShowPreviewAuras()
    if not self.db or not self.db.global.showPreviewAuras then
        if BF._debugContainerPreview then print("|cffff0000[BF ShouldShow]|r showPreviewAuras is false/nil") end
        return false
    end
    ...
    local ACD = LibStub("AceConfigDialog-3.0", true)
    local aceFrame = ACD and ACD.OpenFrames and ACD.OpenFrames["BuzzardFrames"]
    if not aceFrame or not aceFrame.frame or not aceFrame.frame:IsShown() then
        if BF._debugContainerPreview then print("|cffff0000[BF ShouldShow]|r aceFrame not open") end
        return false
    end
...
local function RefreshPreview(self, _unused)
    ...
    if BF._previewOOR then
        ...
        -- v28: healthPower range-fade/desaturate keys are per-flat under
        -- the healthPower per-layout toggle. Resolve per-frame via
        -- GetSectionProfile so each preview respects its own flat's
        -- range settings. fadeOfflineFrames is likewise per-flat under
        -- the text per-layout toggle.
        if BF._previewFrames then
            for flatID, f in pairs(BF._previewFrames) do
                if f:IsShown() then
                    local flat = GetFlatProfile(flatID)
                    local hp   = BF:GetSectionProfile("healthPower", flat) or {}
                    local tp   = BF:GetSectionProfile("text", flat)
                    local fadeOff      = tp and tp.fadeOfflineFrames
                    local fadeAlpha    = hp.rangeFadeAlpha or 0.4
                    local rangeEnabled = hp.enableRangeFade
                    local desat = hp.enableRangeDesaturate and (hp.rangeDesaturation or 1.0) or 0
                    local alpha = rangeEnabled
                        and ((isOffline and not fadeOff) and 1.0 or fadeAlpha)
                        or 1.0
                    f:SetAlpha(alpha)
                    BF:DarkenPreviewFrame(f, desat)
                end
            end
        end
        -- CF previews aren't per-flat; fall back to the global for both
        -- healthPower and text via nil-flat.
        local cfHP     = BF:GetSectionProfile("healthPower", nil) or {}
        local cfTP     = BF:GetSectionProfile("text", nil)
        local cfFadeOff = cfTP and cfTP.fadeOfflineFrames
        local cfFade   = cfHP.rangeFadeAlpha or 0.4
        local cfRange  = cfHP.enableRangeFade
        local cfDesat  = cfHP.enableRangeDesaturate and (cfHP.rangeDesaturation or 1.0) or 0
        ...
        if BF._previewCFFrames then
            for i = 1, #enabledCFs do
                ...
                if cf then
                    cf:SetAlpha(cfAlpha)
                    BF:DarkenPreviewFrame(cf, cfDesat)
                end
            end
        end
    else
        ...
        if BF._previewCFFrames then
            ...
            for slot, entry in ipairs(enabledCFsForAuras) do
                ...
                if cf and cf:IsShown() then
                    cf._bf_containerGroupTypeKey = "cfGroup_" .. entry.index
...
-- bigDef, important, crowdControl, privateAuras, stack text).
--
...
function BF:RefreshPreviewDummyImportant()
    _ForEachShownPreviewFrameAuraCtx(function(frame, isRaid, flat)
        if isRaid then
            BF:ShowDummyImportant(frame, true, flat, nil)
        else
            BF:ShowDummyImportant(frame, false, nil, flat)
        end
    end)
end

function BF:RefreshPreviewDummyCrowdControl()
    _ForEachShownPreviewFrameAuraCtx(function(frame, isRaid, flat)
        if isRaid then
            BF:ShowDummyCrowdControl(frame, true, flat, nil)
        else
            BF:ShowDummyCrowdControl(frame, false, nil, flat)
        end
    end)
end

function BF:RefreshPreviewDummyPrivateAuras()
    _ForEachShownPreviewFrameAuraCtx(function(frame, isRaid, flat)
        if isRaid then
            BF:ShowDummyPrivateAuras(frame, true, flat, nil)
        else
            BF:ShowDummyPrivateAuras(frame, false, nil, flat)
        end
    end)
end
...
-- settings for buffs + debuffs + bigDef + important + crowdControl + private
-- auras at once, so the preview needs all five non-stack types repainted.
...
local _globalTypeFlags = {
    ...
    important = true, crowdControl = true, privateAuras = true,
}
...
-- Per-container preview refresh used by Options_AuraCustomizations.lua's
-- DebouncedCCRefresh for threshold/color slider + picker edits. Repaints
