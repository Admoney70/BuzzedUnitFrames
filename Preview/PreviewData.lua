...
-- falls back to the global -- which is the correct behavior since CF
...
-- v35: dispel indicator sub-category. Used by ApplyPreviewHighlights for
-- the debuff border/overlay keys that migrated from borders to
-- auras.dispelIndicator in v35.
--
-- v60: resolved per sub-category instead of _rp_section("auras") plus an
-- index. _rp_section routes through the coarse IsPerLayoutSection("auras")
-- alias, which answers true when EITHER aura group is per-layout -- so with
-- Buffs per-layout ON and Debuffs OFF the preview would have painted the
-- dispel border/overlay from the flat's stale dispelIndicator rawkeys while
-- live frames correctly read the global.
local function _rp_dispelIndicator()
    if _currentPreviewFrame and _currentPreviewFrame._cfgFlat then
        return BF:GetAurasSubcatProfileForFrame("dispelIndicator", _currentPreviewFrame) or {}
    end
    return BF:GetAurasSubcatProfile("dispelIndicator", _currentPreviewFlat) or {}
end
...
-- by ComputeActivePreviewSet in PreviewSystem.lua). The Nth
...
-- PreviewSystem.lua (seeded flats first in fixed order, then
...
local function GetFakeUnit(flatID)
    -- A frame that carries its own unit (standalone preview frames --
    -- the boss preview's twin stand-ins, PreviewSystem.lua
    -- PaintStandalonePreviewFrame) is painted from that, not from the
    -- flat's fake player. Every painter sets _currentPreviewFrame through
    -- _setPreviewFlat before asking, so the override is always in scope
    -- here. Fields: name, class (nil = not a player), hp, role ("NONE" =
    -- no group role).
    local own = _currentPreviewFrame and _currentPreviewFrame._bf_previewUnit
...
local function LerpGradientColor(hp, lowColor, midColor, highColor)
    local lowA  = lowColor.a  or 1
    local midA  = midColor.a  or 1
    local highA = highColor.a or 1
    if hp <= 0 then
        return lowColor.r, lowColor.g, lowColor.b, lowA
    elseif hp >= 1 then
        return highColor.r, highColor.g, highColor.b, highA
    elseif hp <= 0.5 then
        local t = hp / 0.5
...
local function ApplyPreviewHealthBar(frame, isRaid, p, tier)
    ...
    -- Apply fill opacity. In gradient mode, alpha comes from the gradient
    -- color stops; otherwise the healthBarOpacity slider applies.
    local fillAlpha = isGradient and gradientAlpha or _rp_healthPower().healthBarOpacity or 1
    ...
    -- or static — reset SetAlpha on every refresh so toggling between
    -- gradient and static modes restores the correct rendered alpha
    -- without needing a full preview re-layout.
    if frame.healthBar.bg then
        local hp = _rp_healthPower()
        if hp.useCustomBackgroundColor and hp.useBgGradient then
            local br, bg_, bb, ba = LerpGradientColor(unit.hp,
                _rp_colors().bgGradientLow, _rp_colors().bgGradientMid, _rp_colors().bgGradientHigh)
            frame.healthBar.bg:SetColorTexture(br, bg_, bb, ba)
            frame.healthBar.bg:SetAlpha(1)
        elseif hp.useCustomBackgroundColor and hp.useBgClass then
            -- Class mode: paint the bg with the fake unit's class color.
            -- Always darkened (hasCustomTexture=false), matching the live
            -- HealthBarColor:Update path; backgroundAlpha drives alpha.
            local br, bg_, bb
            if fakeClassColor then
                br, bg_, bb = BF:GetClassHealthColor(fakeClassColor.r, fakeClassColor.g, fakeClassColor.b, false)
            elseif isRaid then
                br, bg_, bb = BF:GetClassHealthColor(0.0, 0.44, 1.0, false)
            else
                br, bg_, bb = BF:GetClassHealthColor(0.0, 0.65, 0.0, false)
            end
            -- Background Darkening: multiply RGB toward black by the slider.
            local d = 1 - (hp.bgClassDarken or 0)
            frame.healthBar.bg:SetColorTexture(br * d, bg_ * d, bb * d, 1)
            frame.healthBar.bg:SetAlpha(hp.backgroundAlpha or 1)
        elseif hp.useCustomBackgroundColor then
            local c = hp.backgroundColor or { r = 0, g = 0, b = 0 }
            frame.healthBar.bg:SetColorTexture(c.r, c.g, c.b, 1)
            frame.healthBar.bg:SetAlpha(hp.backgroundAlpha or 1)
        else
...
function BF:ResetPreviewHealthColor(frame)
    -- v94: accept EITHER preview identity. The gate tested _previewTier alone,
    -- which only Custom Frame Group preview frames carry -- flat (Raid/Party)
    -- previews are stamped with _flatID instead -- so this early-returned on
    -- every flat preview frame and the "always reset to default first"
    -- contract its callers rely on was silently dead there. The body already
    -- PREFERS _flatID (see _setPreviewFlat below), so the gate was simply
    -- narrower than the function.
    if not frame or not frame.healthBar then return end
    if not (frame._flatID or frame._previewTier) then return end
...
local function ApplyPreviewName(frame, isRaid, p, tier)
    ...
    -- Cyrillic font handling (mirrors NameText:Update). REFACTOR (plan D1):
    -- reads the fs-scoped config cache written by NameText:Layout; the old
    -- per-frame parent._nameFont* snapshots are gone.
    local fsCfgPath  = frame.nameText and frame.nameText._cfgFontPath
    local fsCfgSize  = (frame.nameText and frame.nameText._cfgFontSize) or _rp_text().nameFontSize or 10
    local fsCfgFlags = (frame.nameText and frame.nameText._cfgFontFlags) or ""
    if BF.HasCyrillic and BF:HasCyrillic(name) then
        if _rp_text().transliterateCyrillicNames and BF.TransliterateCyrillic then
            ...
            if fsCfgPath then
                frame.nameText:SetFont(fsCfgPath, fsCfgSize, fsCfgFlags)
            end
        else
            -- Use Cyrillic fallback font
            local fallback = BF.cyrillicFont or BF.font
            frame.nameText:SetFont(fallback, fsCfgSize, fsCfgFlags)
        end
    elseif fsCfgPath then
    ...
    if _rp_text().adjustNameColors then
        if _rp_text().classColorNames and fakeClassColor then
            ...
            frame.nameText:SetTextColor(nc.r, nc.g, nc.b, nc.a or 1)
        end
    else
...
local function ApplyPreviewHealthText(frame, isRaid, p, tier)
    ...
    if _rp_text().adjustHealthTextColor then
        if _rp_text().classColorHealthText and fakeClassColor then
            ...
            frame.healthText:SetTextColor(c.r, c.g, c.b, c.a or 1)
        end
    else
        frame.healthText:SetTextColor(1, 1, 1)
    end
    local tp = _rp_text()
    local fmt = tp.healthTextFormat or "percent"
    local pct = math.floor(unit.hp * 100 + 0.5)
    local deficit = hdata.max - hdata.current
    local pctStr = (tp.healthTextPctSymbol ~= false) and (pct .. "%") or tostring(pct)
    if     fmt == "percent" then frame.healthText:SetText(pctStr)
    ...
    else                         frame.healthText:SetText(pctStr) end
...
local function ApplyPreviewPowerBar(frame, isRaid, p, tier)
    ...
    -- A frame's own unit (standalone previews) may carry `forcePower`: the
    -- twin rule (BF:TwinForcesPowerBar) -- the oUF frame's power bar switch
    -- decides, the three filters are skipped.
    local showPower = false
    if unit.forcePower ~= nil then
        showPower = unit.forcePower and true or false
    elseif _rp_healthPower().showAllPowerBars then
        showPower = true
    else
...
local function ApplyPreviewRoleIcon(frame, isRaid, p, tier)
    ...
    -- A unit with no group role (an NPC): no icon, name takes the inset.
    ...
    elseif style == "GLASS" then
        ...
        local file = role == "TANK" and "role_glass_tank"
                  or role == "HEALER" and "role_glass_healer"
                  or "role_glass_dps"
        tex:SetTexture("Interface\\AddOns\\BuzzardFrames\\Media\\" .. file)
    else
...
local function ApplyPreviewAggro(frame, isRaid, p, tier)
    ...
    if _rp_borders().aggroEnabled and BF.db.global.testAggroHighlight and CFModuleFlag(frame, "moduleShowAggroHighlight") then
        ...
        -- Rounded borders can't use the Blizzard aggro atlas — show Border.
        if (style == "blizzard" or style == "glowBorder")
            and _rp_borders().enableBorder ~= false
            and BF.IsRoundedBorderStyle and BF.IsRoundedBorderStyle(_rp_borders().borderStyle) then
            style = "border"
        end
        ...
        -- Rounded-aware: draws the ring (at the aggro border width) for
        -- rounded frame styles when the aggro style is "border"; alpha 0
        -- (any non-border style) hides it.
        BF:SetHighlightBorder(a, frame, pc.r, pc.g, pc.b,
            style == "border" and alpha or 0, bp.aggroBorderWidth or 2)
        local awPx = math.max(2, math.min(5, bp.aggroBorderWidth or 2))
        if a.blizzardParts then
            local bzw   = math.max(1, math.min(5, bp.aggroBorderWidth or 2))
            local MEDIA = "Interface\\AddOns\\BuzzardFrames\\Media\\"
            local P2U   = (BF.PixelsToUI and function(n) return BF:PixelsToUI(n) end) or function(n) return n end
            local cs, band = P2U(bzw + 5), P2U(bzw + 2)
            local bzp, bzAlpha = a.blizzardParts, (style == "blizzard") and alpha or 0
            -- 32px tiles, art in the first (w+5) texels -- crop so 1 texel
            -- renders as 1 pixel (see Indicators/AggroHighlight.lua:Layout).
            local TILE, uv = 32, (bzw + 5) / 32
            local ev = (bzw + 2) / 32
            for _, k in ipairs({ "tl", "tr", "bl", "br" }) do
                bzp[k]:SetTexture(MEDIA .. "AggroCorner" .. bzw)
                bzp[k]:SetSize(cs, cs); bzp[k]:ClearAllPoints()
            end
            bzp.tl:SetTexCoord(0,  uv, 0,  uv)
            bzp.tr:SetTexCoord(uv, 0,  0,  uv)
            bzp.bl:SetTexCoord(0,  uv, uv, 0 )
            bzp.br:SetTexCoord(uv, 0,  uv, 0 )
            bzp.tl:SetPoint("TOPLEFT",     frame, "TOPLEFT")
            bzp.tr:SetPoint("TOPRIGHT",    frame, "TOPRIGHT")
            bzp.bl:SetPoint("BOTTOMLEFT",  frame, "BOTTOMLEFT")
            bzp.br:SetPoint("BOTTOMRIGHT", frame, "BOTTOMRIGHT")
            bzp.top:SetTexture(MEDIA .. "AggroEdgeH" .. bzw); bzp.top:ClearAllPoints()
            bzp.top:SetTexCoord(0, 1, 0, ev)
            bzp.top:SetPoint("TOPLEFT", bzp.tl, "TOPRIGHT"); bzp.top:SetPoint("TOPRIGHT", bzp.tr, "TOPLEFT")
            bzp.top:SetHeight(band)
            bzp.bottom:SetTexture(MEDIA .. "AggroEdgeH" .. bzw); bzp.bottom:ClearAllPoints()
            bzp.bottom:SetTexCoord(0, 1, ev, 0)
            bzp.bottom:SetPoint("BOTTOMLEFT", bzp.bl, "BOTTOMRIGHT"); bzp.bottom:SetPoint("BOTTOMRIGHT", bzp.br, "BOTTOMLEFT")
            bzp.bottom:SetHeight(band)
            bzp.left:SetTexture(MEDIA .. "AggroEdgeV" .. bzw); bzp.left:ClearAllPoints()
            bzp.left:SetTexCoord(0, ev, 0, 1)
            bzp.left:SetPoint("TOPLEFT", bzp.tl, "BOTTOMLEFT"); bzp.left:SetPoint("BOTTOMLEFT", bzp.bl, "TOPLEFT")
            bzp.left:SetWidth(band)
            bzp.right:SetTexture(MEDIA .. "AggroEdgeV" .. bzw); bzp.right:ClearAllPoints()
            bzp.right:SetTexCoord(ev, 0, 0, 1)
            bzp.right:SetPoint("TOPRIGHT", bzp.tr, "BOTTOMRIGHT"); bzp.right:SetPoint("BOTTOMRIGHT", bzp.br, "TOPRIGHT")
            bzp.right:SetWidth(band)
            for _, k in ipairs({ "tl", "tr", "bl", "br", "top", "bottom", "left", "right" }) do
                bzp[k]:SetVertexColor(pc.r, pc.g, pc.b)
                bzp[k]:SetAlpha(bzAlpha)
            end
        end
        if a.glowBorder then
            a.glowBorder:SetTexture("Interface\\AddOns\\BuzzardFrames\\Media\\aggro_border_glow_v3_" .. awPx .. "px")
            a.glowBorder:SetVertexColor(pc.r, pc.g, pc.b)
            a.glowBorder:SetAlpha(style == "glowBorder" and alpha or 0)
        end
        ...
        if a.arrowTex then
            if style == "arrow" or style == "arrowSingle" then
                ...
                -- Counter-clockwise from a right-pointing texture, so a
                -- quarter turn is UP. Kept identical to the indicator's
                -- own mapping (Indicators/AggroHighlight.lua) -- the two
                -- were inverted together and are corrected together.
        ...
        BF:SetHighlightBorder(a, frame, 0, 0, 0, 0)
        if a.blizzardParts then
            for _, k in ipairs({ "tl", "tr", "bl", "br", "top", "bottom", "left", "right" }) do
                a.blizzardParts[k]:SetAlpha(0)
            end
        end
...
local function ApplyPreviewHighlights(frame, isRaid, p, tier)
    ...
    if frame.targetHighlight then BF:SetHighlightBorder(frame.targetHighlight, frame, 0, 0, 0, 0) end
    ...
    -- (must agree with _ShowDummyDispelBorders, which runs after this on the
    -- full-refresh path); setup-mode test frames keep the old global toggle.
    local simDispel
    if frame._isPreviewFrame then
        local _, _, showDebuffs, sd = BF:GetActivePreviewMode()
        simDispel = showDebuffs and sd or false
    else
        -- Setup-mode test frames always simulate dispellable (old default was
        -- on; the simulateDispellableDebuff toggle is retired in v70).
        simDispel = true
    end
    if frame.dispelDebuffBorder then
        if blizzardActive and simDispel and CFModuleFlag(frame, "moduleShowDebuffBorder") then
            ...
            BF:SetHighlightBorder(dh, frame, 0.20, 0.60, 1.00, 1, 3)
        -- (testDebuffBorder arm REMOVED — setting was never exposed in the
        -- UI; the custom-mode preview is painted by _ShowDummyDispelBorders.)
        else
            BF:SetHighlightBorder(frame.dispelDebuffBorder, frame, 0, 0, 0, 0)
        end
    end
    -- (testDebuffOverlay branch REMOVED — setting was never exposed in the
    -- UI; the custom-mode preview is painted by _ShowDummyDispelBorders.)
    if frame.dispelDebuffOverlay then
        frame.dispelDebuffOverlay:Hide()
    end
    -- The health-color tint too: _ShowDummyDispelBorders repaints it after
    -- this on the full-refresh path, like the overlay above.
...
local function ApplyPreviewStatus(frame, isRaid, p, tier)
    ...
    local function applyOORAlpha(isOffline)
        -- Fade + darken through the ONE shared writer
        -- (BF:ApplyRangeEffects, Indicators/RangeAlpha.lua) so the two
        -- effects cannot diverge. state=false simulates OOR.
        if not BF._previewOOR then
            BF:ApplyRangeEffects(frame, true, true, 1, 0)  -- reset
            return
        end
        local hp = _rp_healthPower()
        local fadeAlpha = hp.rangeFadeAlpha or 0.4
        local alpha = hp.enableRangeFade
            and ((isOffline and not hp.fadeOfflineFrames) and 1.0 or fadeAlpha)
            or 1.0
        local desat = hp.enableRangeDesaturate and (hp.rangeDesaturation or 0.5) or 0
        BF:ApplyRangeEffects(frame, false, true, alpha, desat)
    end
    ...
    if status == "offline" then
        ...
        if _rp_healthPower().useCustomOfflineColor then
            local c = _rp_healthPower().offlineBackgroundColor or { r=0.3, g=0.3, b=0.3 }
    ...
    elseif status == "dead" then
        ...
        frame.healthBar:SetMinMaxValues(0, 1)
        frame.healthBar:SetValue(0)
        local deadBarColor   = (_rp_healthPower().useCustomDeadColor and _rp_healthPower().deadBackgroundColor) or { r=0.1, g=0.1, b=0.1 }
        local deadBarOpacity = _rp_healthPower().useCustomDeadColor and (_rp_healthPower().deadBackgroundOpacity ~= nil and _rp_healthPower().deadBackgroundOpacity or 0.7) or 0.7
        if frame.statusOverlay then
        ...
        if frame.rezPendingIcon then
            if (BF._previewResurrect or BF.db.global.testResurrectPending) and _rp_icons().showResurrectPending then
                ...
                local siSize = _rp_icons().statusIconSize or 24
                if style == "buzzard" then
                    ...
                    frame.rezPendingIcon:SetTexture("Interface\\AddOns\\BuzzardFrames\\Media\\buzzardRes", nil, nil, "TRILINEAR")
                    ...
                    frame.rezPendingIcon:SetSize(siSize * 0.85, siSize * 0.85)
                else
                    frame.rezPendingIcon:SetTexture(nil)
                    frame.rezPendingIcon:SetTexCoord(0, 1, 0, 1)
                    frame.rezPendingIcon:SetAtlas("RaidFrame-Icon-Rez")
                    frame.rezPendingIcon:SetSize(siSize, siSize)
                end
        ...
        if BF._previewOOR then
            local f = _rp_healthPower().deadColorOORFactor; if f == nil then f = 0.5 end
            local gray = (tc.r + tc.g + tc.b) / 3
            tc = { r = tc.r*f + gray*(1-f), g = tc.g*f + gray*(1-f), b = tc.b*f + gray*(1-f) }
        end
        if _rp_text().appendStatusTextToNames then
            if frame.statusText then frame.statusText:Hide() end
            if frame.nameText and _rp_text().showName then
...
local function ApplyPreviewAbsorbOverlay(frame, isRaid, p, tier)
    ...
    if frame.absorbMissingHealth then
        frame.absorbMissingHealth:Hide()
        if frame.absorbMissingHealth.leftShadow then frame.absorbMissingHealth.leftShadow:Hide() end
    end
    ...
    if frame.absorbMissingHealth and hFill then
        ...
        elseif _rp_absorbs().overshieldStyle ~= "Glow" and overshieldActive and ovAnchor == "LEFT" then
            ...
            -- Left shadow (absorb/health seam). Independent of the custom
            -- texture toggle -- mirrors Indicators/AbsorbBars.lua.
            local pls = frame.absorbMissingHealth.leftShadow
            if pls then
                if _rp_absorbs().showAbsorbShadow ~= false then
                    local sc = _rp_absorbs().absorbShadowColor
                    pls:SetVertexColor(sc and sc.r or 1, sc and sc.g or 1,
                        sc and sc.b or 1, sc and sc.a or 1)
                    pls:SetAlpha(1)
                    pls:Show()
                else
                    pls:Hide()
...
local function ApplyPreviewHealPrediction(frame, isRaid, p, tier)
    ...
    if hBar then
        ...
        if anchor == "LEFT" then
            ...
            frame.healPrediction:SetFrameLevel(hBar:GetFrameLevel() + 6)
        elseif hFill then
...
local function ApplyPreviewHealAbsorb(frame, isRaid, p, tier)
    ...
    -- v93: custom heal-absorb fill texture (base fill only), mirroring
    -- AbsorbBars:_ApplyAbsorbStyle so Setup Mode matches live frames.
    local haCustomTex = _rp_absorbs().useCustomHealAbsorbTexture
        and BF:ResolveBarTexture(_rp_absorbs().healAbsorbTexture or "Solid") or nil
    -- v93: plus-symbol tint (see the twin note in AbsorbBars).
    local hsc = (_rp_absorbs().useCustomHealAbsorbSymbolColor
        and _rp_absorbs().healAbsorbSymbolColor) or { r = 1, g = 1, b = 1, a = 0.2 }
    ...
    if style == "Bar" or style == "BarBlizzard" then
        if frame.healAbsorbBar and frame.healthBar then
            ...
            if style == "BarBlizzard" then
                ...
                if frame.healAbsorbBar._overlay then
                    frame.healAbsorbBar._overlay:SetVertexColor(hsc.r, hsc.g, hsc.b, hsc.a or 0.2)
                    frame.healAbsorbBar._overlay:Show()
                end
            else
                if tex then
            ...
            -- v93: match AbsorbBars' +8 -- at +10 the strip drew over the
            -- frame border (parent+10) on preview frames.
            frame.healAbsorbBar:SetFrameLevel(frame.healthBar:GetFrameLevel() + 8)
        ...
        if frame.healAbsorb then
            ...
            if style == "OverlayBlizzard" then
                ...
                if frame.healAbsorbOverlay then
                    frame.healAbsorbOverlay:SetVertexColor(hsc.r, hsc.g, hsc.b, hsc.a or 0.2)
                    frame.healAbsorbOverlay:Show()
                end
...
-- ============================================================
-- Exports: expose local functions for PreviewSystem.lua
-- ============================================================
...
-- Drops the painter context a standalone paint left behind, so a later
-- GetFakeUnit from outside a painter (the Preview Units tab) cannot read
-- that frame's own unit.
BF._preview.ClearPreviewContext = function()
    _currentPreviewFrame = nil
    _currentPreviewFlat  = nil
end
