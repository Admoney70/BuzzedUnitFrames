-- ============================================================
-- SALVAGED oUF ELEMENT -- BuzzardFrames-owned, NOT part of the library.
--
-- Source: oUF `elements/healthprediction.lua`, vendored verbatim from the
-- aurapocalypse build (8b10030, 2026-08-06). Upstream DELETED this element in
-- oUF 14.0.0 ("healthprediction: Remove element"), folding the feature into
-- Health sub-widgets (Health.HealingAll, Health.DamageAbsorb, ... capitalised).
--
-- Kept because BuzzardFrames wires `frame.HealthPrediction` in
-- UnitFrames/oUF_Absorbs.lua; without the element that table is inert and all
-- heal-prediction / damage-absorb / overshield art silently disappears.
--
-- Verified safe against 14.0.0: this file touches only `self.Health` (still a
-- StatusBar), `self.__unit` (unchanged in 14.0.0), StatusBar methods, and
-- `element.values`, which it creates itself. It does NOT read anything the
-- 14.0.0 "keep state internal" sweep moved out of reach (Health.cur/.max,
-- SetColorDisconnected, __isHoriz/__size).
--
-- It lives HERE, not in Libs/oUF, so the vendored library stays a pristine
-- upstream copy -- see Libs/oUF/CHANGELOG.md. Registration is via the public
-- extension API (`local _, ns = ...; ns.oUF` + `oUF:AddElement`), which works
-- from any BuzzardFrames file loaded after Libs\oUF\oUF.xml.
--
-- CODE BELOW IS UNMODIFIED. Keep it that way, so a future upstream diff is
-- still meaningful. Migrating to Health sub-widgets would let this file go.
-- ============================================================
