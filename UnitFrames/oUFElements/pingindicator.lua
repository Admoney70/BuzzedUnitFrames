...
--
-- Source: oUF `elements/pingindicator.lua`, vendored verbatim from the
-- aurapocalypse build (8b10030, 2026-08-06). The element is absent from oUF
-- 14.0.0.
--
-- Kept because BuzzardFrames wires `self.PingIndicator` in
-- UnitFrames/oUF_Shared.lua and exposes it as a user setting
-- (oufShowPingIndicator, default ON). Without the element, EnableElement
-- silently no-ops on the unknown name and the ping pin never shows.
--
-- Verified safe against 14.0.0: touches only `self.__unit` and
-- `element.__owner`, both unchanged.
--
-- It lives HERE, not in Libs/oUF, so the vendored library stays a pristine
-- upstream copy -- see Libs/oUF/CHANGELOG.md.
--
-- CODE BELOW IS UNMODIFIED. Keep it that way.
-- ============================================================
...
.useAtlasSize - Makes the element use preprogramd atlas' size instead of its set dimensions (boolean)
...
local function Update(self, event, unitGUID, textureKit)
	if(issecretvalue(unitGUID)) then
		return
	elseif(unitGUID ~= (self.unitGUID or UnitGUID(self.__unit))) then
		return
	end
